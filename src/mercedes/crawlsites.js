 ...
 */
 ...
 const URL = process.env.URL || 'https://www.mercedes-benzworld.co.uk/';
 const DEPTH = parseInt(process.env.DEPTH) || 1;
 ...
 function slugify(str) {
   return str.replace(/http?s:\/\//g, '').replace(/[\/:]/g, '_');
 }
  ...
  */
 function collectAllSameOriginAnchorsDeep(sameOrigin = false) {
   const allElements = [];
 ...
 async function crawl(browser, page, depth = 0) {
   ...
   if (crawledPages.has(page.url)) {
     ...
     newPage.on('request', interceptedRequest => {
       ...
       if (['.png','.jpg','.webp'].some((item) => requestURL.includes(item))) {
         interceptedRequest.abort();
         return false;
       }
 
       if (requestURL.includes('dls')) {
         page.dls = true;
       }
 
       if (requestURL.includes('launch')) {
         page.launch = true;
       }
 
       if (requestURL.includes('/satelliteLib-')) {
         page.dtm = true;
       }
